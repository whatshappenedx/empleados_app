    ...
    <div class="absolute inset-0 flex items-center justify-center p-4">
      <div class="w-full max-w-lg bg-white dark:bg-slate-800 dark:text-slate-100 rounded-xl shadow-2xl border border-brand-mid/50 animate-pop-in">
        <div class="flex items-center justify-between p-4 border-b border-brand-mid/40">
          <h3 class="text-lg font-semibold">{{ model.id ? 'Editar empleado' : 'Nuevo empleado' }}</h3>
        ...
        <form (ngSubmit)="save()" #f="ngForm" class="p-4 space-y-4" aria-modal="true" role="dialog">
          <div>
            ...
            <input [(ngModel)]="model.nombre" name="nombre" required
          ...
          <div>
            ...
            <input [(ngModel)]="model.correo" name="correo" type="email" required
                   class="border border-brand-mid px-3 py-2 rounded w-full focus:ring-2 focus:ring-brand-dark dark:bg-slate-900 dark:border-slate-600"
          ...
          <div>
            ...
            <input [(ngModel)]="model.cargo" name="cargo" required
                   class="border border-brand-mid px-3 py-2 rounded w-full focus:ring-2 focus:ring-brand-dark dark:bg-slate-900 dark:border-slate-600"
          ...
          <div>
            ...
            <input [(ngModel)]="model.fecha_ingreso" name="fecha_ingreso" placeholder="YYYY-MM-DD" pattern="^\\d{4}-\\d{2}-\\d{2}$"
                   class="border border-brand-mid px-3 py-2 rounded w-full focus:ring-2 focus:ring-brand-dark dark:bg-slate-900 dark:border-slate-600"
...
export class EmployeeModalComponent implements OnChanges {
  ...
  model: Employee = { nombre: '', correo: '', cargo: '' };
  loading = false;
  ...
  ngOnChanges(changes: SimpleChanges): void {
    if (changes['employee']) {
      this.model = this.employee ? { ...this.employee } : { nombre: '', correo: '', cargo: '' };
    }
  }

  onClose() { if (!this.loading) { this.closed.emit(); this.model = { nombre: '', correo: '', cargo: '' }; } }

  save() {
    this.loading = true;
    this.api.saveEmployee(this.model).subscribe({
      next: () => {
        this.toast.success(this.model.id ? 'Empleado actualizado' : 'Empleado creado');
        this.loading = false;
        this.saved.emit();
        this.onClose();
      },
